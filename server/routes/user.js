    ...
    jwt.verify(req.query.token, 'secret', function (err, decoded) {
        ...
        User.findById(decoded.user._id, function (err, user) {
            ...
            for (let uc of user.cart) {
                if (String(uc.productId) == String(cart.productId) && String(uc.style) == String(cart.style)) {
    ...
    jwt.verify(req.query.token, 'secret', function (err, decoded) {
        ...
        User.findById(decoded.user._id, function (err, user) {
            ...
            for ( let i = 0; i < user.cart.length; i++ ) {
                if (String(user.cart[i].productId) == String(cart.productId) && String(user.cart[i].style) == String(cart.style)) {
                    console.log("Before \n\n" + user.cart);
                    let uc = user.cart[i];
